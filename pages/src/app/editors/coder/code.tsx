...
export default function ({ editConfig, env }: any): JSX.Element {
  ...
  const onClose = () => {
    ...
    setModel(c => ({ ...c, iconsVisible: false }))
    setModalContext(c => ({ ...c, visible: false }))
  };
  ...
  const open = useCallback(() => {
    if (!modalContext.visible) {
      setModalContext(c => ({ ...c, visible: true  }))
      popView(
        model.title,
        () => {
          return (
  ...
  }, [model, readonly, comments, modalContext.visible]);
  ...
  }, [modalContext.visible, render, model, model.val]);
